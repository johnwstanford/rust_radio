...
use rust_radio::gnss::gps_l1_ca::tracking::algorithm_b;
...
fn main() {
	...
	let mut trk = algorithm_b::new_default_tracker(prn, 0.0, fs, 40.0, 4.0);
	let mut code_phase:usize = 0;
	let mut all_results:Vec<algorithm_b::TrackingDebug> = vec![];
	...
	'outer_acq: for s in io::file_source_i16_complex(&fname).map(|(x, idx)| (Complex{ re: x.0 as f64, im: x.1 as f64 }, idx)) {
		...
		match acq.block_for_result(prn) {
			Ok(Some(result)) => {
				...
				trk = algorithm_b::new_default_tracker(prn, result.doppler_hz, fs, 40.0, 4.0);
	...
	'outer_trk: for s in io::file_source_i16_complex(&fname).map(|(x, idx)| (Complex{ re: x.0 as f64, im: x.1 as f64 }, idx)).skip(code_phase) {
		...
		match trk.apply(s) {
			algorithm_b::TrackingResult::Err(_) => break 'outer_trk,
			_                                   => {},
		}
		...
		if s.1 % 2000000 == 0 {
			let debug = trk.debug();
			match trk.state {
				algorithm_b::TrackingState::WaitingForInitialLockStatus => eprintln!("WaitingForInitialLockStatus {}", format!("{:9.2} [Hz], {:14.3}", debug.carrier_hz, debug.estimated_snr_coh).yellow()),
				algorithm_b::TrackingState::WaitingForFirstTransition   => eprintln!("WaitingForFirstTransition {}", format!("{:9.2} [Hz], {:14.3}", debug.carrier_hz, debug.estimated_snr_coh).yellow()),
				algorithm_b::TrackingState::Tracking                    => eprintln!("Tracking {}", format!("{:9.2} [Hz], {:9.2}", debug.carrier_hz, debug.estimated_snr_coh).green()),
			}
